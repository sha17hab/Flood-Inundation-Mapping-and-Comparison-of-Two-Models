...
if (res(DEM_Flood1_0)[1]!=res(DEM_Flood2_0)[1]) {
  if (res(DEM_Flood1_0)[1]<res(DEM_Flood2_0)[1]) {
    DEM_Flood2 = resample(DEM_Flood2_0,DEM_Flood1_0,method='ngb')
  } else DEM_Flood1 = resample(DEM_Flood1_0,DEM_Flood2_0,method='ngb')
...
rm(DEM_Flood1_0);rm(DEM_Flood2_0)
...
partition_frame_size = 790 # number of cell in a row or column
...
pdf("mygraph.pdf",paper = "USr",width = 10,height = 10)
...
legend( par()$usr[1]+500, #x-coordinate of upper-left corner of legend box
        ...
        legend = c("Dry Zone", 
                   expression(paste("Model1 ", intersect(" ") ,"(Model1 ",union(" "), "Model2)")), 
                   expression(paste("Model2 ",intersect(" ") ,"(Model1 ",union(" "),"Model2)")), 
                   expression(paste("Model1 ",intersect(" ") ,"Model2"))),
...
writeRaster(DEM_FloodDepthIntersect, filename="test_intersct.tif", format="GTiff", overwrite=TRUE)
writeRaster(DEM_FloodDepthClassified_1, filename="test_1.tif", format="GTiff", overwrite=TRUE)
writeRaster(DEM_FloodDepthClassified_2, filename="test_2.tif", format="GTiff", overwrite=TRUE)

hist(DEM_FloodDepthIntersect, main="Distribution of elevation values", 
     col= "purple", 
     maxpixels=ncell(DEM_FloodDepthIntersect))
#############################################################

plot(DEM_FloodDepthUnion)
plot(DEM_FloodDepthIntersect)
plot(DEM_FloodDepthClassified_1)
plot(DEM_FloodDepthClassified_2)
